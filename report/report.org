...
** Available tools
   ...
   of \emph{First Order Logic} (FOL) and then solve, using any appropriate FOL-theorem
   prover. One of more known such provers is \emph{E-Prover} originally
...
*** Generating Goal and Problem Files
    \label{sec:goal-gen}
...
*** Plan-pruning
    ...
    the \hbox{'A -> B -> ... -> B'} list of states can be reduced to \hbox{'A -> B'.}

*** First Order Logic
    \label{sec:future-fol}
    \todo{Ambiguous description! Not clear if the e-prover or the metric-ff
    was the slow. Rewrite this chapter!}
