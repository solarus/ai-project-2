...
**** Generating a PDDL problem                                        :david:
     ...
     The last step before giving handing over the responsibility to
     the planner is to generate a =PDDL= problem file. This file
     contains the initial or current state of the world encoded using
     the predicates found in the \shrdlite grammar in appendix
     \ref{sec:shrdlite-dom}.
...
*** Plan-pruning
    ...
    consequitve state end up in a state that was reached earlier. In this case
    a these steps are superflous and can be removed.

    A basic form of this pruning was implemented in the current
    project which can be further improved. Currently it only checks if a
    consequtive pick and drop commands happen on the same column. The more
    advanced form of this pruning would be to go through the whole list of
    commands and check if the world state that this command leads was reached
    earlier. If it was, then the list of commands can be deleted back to this
    earlier state. For instance, the 'A -> B -> ... -> B' list of states can
    be reduced to 'A -> B'.

*** First Order Logic
    After some fruitless experimentation with a FOL planner, the so-called
    E-prover, ff-metric was chosen for planning. This decision was based on
    its significantly higher running time and hardship of goal interpretation.
    However, in the long run E-prover might produce more appealling
    results with a simplier problem description form.
...
*** Generating FOL to STRIPS
    ...
    endowed by easy goal generation and provide effortless extenasibility for
    future grammar. Unfortunately, there was no time for experimentations in
    this field, and, therefore, it remained only an idea.
